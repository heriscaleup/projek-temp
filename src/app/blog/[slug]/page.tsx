...
import imageCreditData from "@/data/blogImageCredits.json";
...
const imageCredits = imageCreditData as Record<string, ImageCredit>;
...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return { title: { absolute: post.seoTitle ?? post.title }, description: post.excerpt, keywords: post.tags, authors: [{ name: post.author ?? "Raja Freeze Dried Food" }], alternates: { canonical: `/blog/${slug}` }, openGraph: { title: post.title, description: post.excerpt, type: "article", publishedTime: post.date, modifiedTime: post.updated ?? post.date, authors: [post.author ?? "Raja Freeze Dried Food"], tags: post.tags, images: [{ url: absoluteImage, alt: post.imageAlt ?? post.title }], url: `/blog/${slug}` }, twitter: { card: "summary_large_image", title: post.title, description: post.excerpt, images: [absoluteImage] } };
}
...
export default async function BlogPostDetail({ params }: PageProps) {
  ...
  const imageCredit = imageCredits[image];
  const articleJsonLd: WithContext<BlogPosting> = { "@context":"https://schema.org", "@type":"BlogPosting", headline: post.title, datePublished: post.date, dateModified: post.updated ?? post.date, mainEntityOfPage: { "@type":"WebPage", "@id": `${SITE_URL}/blog/${post.slug}` }, image: [absoluteImage], keywords: post.tags?.join(", "), articleSection: post.category, author: { "@type":"Organization", name:post.author ?? "Raja Freeze Dried Food", url:SITE_URL }, contributor: post.reviewer ? { "@type": "Organization", name: post.reviewer } : undefined, publisher: { "@type":"Organization", name:"Raja Freeze Dried Food", logo:{ "@type":"ImageObject", url:`${SITE_URL}/favicon.webp` } }, description: post.excerpt };
        ...
        <article className="mx-auto grid max-w-6xl gap-10 px-4 py-12 sm:px-6 lg:grid-cols-[minmax(0,1fr)_300px] lg:px-8 lg:py-20">
          <div className="min-w-0">
            ...
            <header className="border-b border-black/10 pb-8">
              <div className="mb-5 flex flex-wrap items-center gap-3 text-sm text-neutral-500"><span className="font-black uppercase tracking-[0.18em] text-orange-700">{post.category}</span><span>•</span><time dateTime={post.date}>Terbit {new Date(post.date).toLocaleDateString("id-ID", { year:"numeric", month:"long", day:"numeric" })}</time>{post.updated && post.updated !== post.date && <><span>•</span><time dateTime={post.updated}>Diperbarui {new Date(post.updated).toLocaleDateString("id-ID", { year:"numeric", month:"long", day:"numeric" })}</time></>}<span>•</span><span>{post.readingTime} menit baca</span></div>
              ...
              <div className="mt-7 flex items-center gap-3"><div className="relative h-11 w-11 overflow-hidden rounded-full border border-black/10 bg-white"><Image src="/favicon.webp" alt="Raja Freeze Dried Food" fill sizes="44px" className="object-contain p-1" /></div><div><p className="text-sm font-bold">{post.author ?? "Raja Freeze Dried Food"}</p><p className="text-xs text-neutral-500">Ditinjau oleh {post.reviewer ?? "Tim Produk Raja Freeze Dried Food"}</p></div></div>
            </header>
            <figure className="my-10"><div className="relative aspect-[16/9] overflow-hidden rounded-[2rem] border border-black/10 bg-neutral-100"><Image src={image} alt={post.imageAlt ?? post.title} fill sizes="(min-width: 1024px) 860px, 100vw" className="object-cover" priority /></div>{imageCredit && <figcaption className="mt-3 text-xs leading-5 text-neutral-500">{imageCredit.sourceUrl ? <a href={imageCredit.sourceUrl} target="_blank" rel="noopener noreferrer" className="underline decoration-neutral-300 underline-offset-2 hover:text-neutral-950">{imageCredit.label}</a> : imageCredit.label}{imageCredit.license && <>{" · "}{imageCredit.licenseUrl ? <a href={imageCredit.licenseUrl} target="_blank" rel="noopener noreferrer" className="underline decoration-neutral-300 underline-offset-2 hover:text-neutral-950">{imageCredit.license}</a> : imageCredit.license}</>}</figcaption>}</figure>
